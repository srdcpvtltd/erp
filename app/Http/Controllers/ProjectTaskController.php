...
class ProjectTaskController extends Controller
{
    ...
    private static $stageData = NULL;
    private static $taskData = NULL;
...
//                }
    ...
    public function allBugList($view)
    {
          ...
          if(Auth::user()->type == 'company'){
            $bugs = Bug::where('created_by',\Auth::user()->creatorId())->with(['assignTo','createdBy','bug_status','project'])->get();
          }
          elseif(Auth::user()->type != 'company'){
            if(\Auth::user()->type == 'client'){
              $user_projects = Project::where('client_id',\Auth::user()->id)->pluck('id','id')->toArray();
              $bugs = Bug::whereIn('project_id', $user_projects)->where('created_by',\Auth::user()->creatorId())->with(['assignTo','createdBy','bug_status','project'])->get();
            }
            else{
              $bugs = Bug::where('created_by',\Auth::user()->creatorId())->whereRaw("find_in_set('" . \Auth::user()->id . "',assign_to)")->with(['assignTo','createdBy','bug_status','project'])->get();
            }
          }
          if($view == 'list'){
    ...
    public function taskStage($id)
    {
        if(self::$stageData == null)
        {
            $stage = TaskStage::find($id);
    ...
    public function taskOrderUpdate(Request $request, $project_id)
    {
        ...
        if(\Auth::user()->can('view project task'))
        {
            ...
            if($request->new_stage != $request->old_stage)
            {
                ...
                $new_stage  = $this->taskStage($request->new_stage);
                $old_stage  = $this->taskStage($request->old_stage);
                $last_stage = TaskStage::where('created_by',\Auth::user()->creatorId())->orderBy('order', 'DESC')->first();
                $last_stage = $last_stage->id;

                $task = $this->projectTask($request->id);
    ...
    public function taskGet($task_id)
    {
        if(\Auth::user()->can('view project task'))
        {
            ...
            {
                $html .= '<span class="text-sm">' . $task->taskProgress($task)['percentage'] . '</span>';
            }
