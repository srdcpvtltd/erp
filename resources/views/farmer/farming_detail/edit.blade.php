...
@section('content')
    <div class="row">
        {{ Form::model($farming, array('route' => array('farmer.farmin_detail.update',$farming->id), 'method' => 'PUT','class'=>'w-100')) }}
        <div class="col-12">
                ...
                <div class="card-body">
                    <div class="row">
                        ...
                        <div class="form-group col-md-6">
                            ...
                            {{ Form::text('name', $farming_detail->name, array('class' => 'form-control','required'=>'required')) }}
                        </div>
                        <div class="form-group col-md-6">
                            ...
                            <div class="form-group">
                                ...
                                <select class="form-control select" name="seed_category_id" id="seed_category_id" required placeholder="Select Seed Category">
                                    ...
                                    @foreach($seed_categories as $seed_category)
                                        <option {{$farming_detail->seed_category_id == $farming_detail ? 'selected' : '' }} value="{{ $seed_category->id }}">{{ $seed_category->name }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
