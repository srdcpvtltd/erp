  ...
  <div class="row justify-content-between align-items-center mb-3">
      <div class="col-md-12 d-flex align-items-center justify-content-between justify-content-md-end">
          ...
          @if($invoice->status!=0 && $invoice->getDue() > 0
            ...
            || $company_payment_setting['is_yookassa_enabled'] == 'on' ||$company_payment_setting['is_midtrans_enabled'] == 'on'
                        ...
                        <div class="col text-end">
                            ...
                            <div class="me-4">
                                <small>
                                  ...
                                  {{\App\Models\Utility::dateFormat($settings,$invoice->issue_date)}}<br><br>
                                </small>
                            </div>
                              <small>
                                  <strong>{{__('Due Date')}} :</strong><br>
                                  {{\App\Models\Utility::dateFormat($settings,$invoice->due_date)}}<br><br>
                              </small>
                                  ...
                                  <table class="table mb-0 table-striped">
                                      ...
                                      @foreach($iteams as $key =>$iteam)
                                          ...
                                          <tr>
                                              ...
                                              <td class="text-end">{{\App\Models\Utility::priceFormat($settings,($iteam->price * $iteam->quantity - $iteam->discount) + $totalTaxPrice)}}</td>
                                          </tr>
                                      @endforeach
                                      <tfoot>
                                      <tr>
                                      ...
                                      <tr>
                                          ...
                                          <td class="text-end">{{\App\Models\Utility::priceFormat($settings,$invoice->getSubTotal())}}</td>
                                      </tr>

                                          <tr>
                                              <td colspan="6"></td>
                                              <td class="text-end"><b>{{__('Discount')}}</b></td>
                                              <td class="text-end">{{\App\Models\Utility::priceFormat($settings,$invoice->getTotalDiscount())}}</td>
                                          </tr>
                                      ...
                                      <tr>
                                          ...
                                          <td class="blue-text text-end">{{\App\Models\Utility::priceFormat($settings,$invoice->getTotal())}}</td>
                                      </tr>
                                      <tr>
                                          <td colspan="6"></td>
                                          <td class="text-end"><b>{{__('Paid')}}</b></td>
                                          <td class="text-end">{{\App\Models\Utility::priceFormat($settings,($invoice->getTotal()-$invoice->getDue())-($invoice->invoiceTotalCreditNote()))}}</td>
                                      </tr>
                                      <tr>
                                          <td colspan="6"></td>
                                          <td class="text-end"><b>{{__('Credit Note')}}</b></td>
                                          <td class="text-end">{{\App\Models\Utility::priceFormat($settings,($invoice->invoiceTotalCreditNote()))}}</td>
                                      </tr>
                                      <tr>
                                          <td colspan="6"></td>
                                          <td class="text-end"><b>{{__('Due')}}</b></td>
                                          <td class="text-end">{{\App\Models\Utility::priceFormat($settings,$invoice->getDue())}}</td>
                                      </tr>
                                      </tfoot>
                                  </table>
                              </div>
                    ...
                    <table class="table">
                        ...
                        @if(!empty($invoice->payments) && $invoice->bankPayments)
                            @foreach($invoice->payments as $key =>$payment)
                                ...
                                <tr>
                                    <td>{{\App\Models\Utility::dateFormat($settings,$payment->date)}}</td>
                                    <td>{{\App\Models\Utility::priceFormat($settings,$payment->amount)}}</td>
                            ...
                            @foreach($invoice->bankPayments as $key =>$bankPayment)
                                <tr>
                                    <td>{{\App\Models\Utility::dateFormat($settings,$bankPayment->date)}}</td>
                                    <td>{{\App\Models\Utility::priceFormat($settings,$bankPayment->amount)}}</td>
                      ...
                      <div class="card bg-none card-box">
                          <section class="nav-tabs p-2">
                              @if ((isset($company_payment_setting['is_stripe_enabled']) && $company_payment_setting['is_stripe_enabled'] == 'on') ||
                                   ...
                                   (isset($company_payment_setting['is_yookassa_enabled']) && $company_payment_setting['is_yookassa_enabled'] == 'on') ||
                                   (isset($company_payment_setting['is_midtrans_enabled']) && $company_payment_setting['is_midtrans_enabled'] == 'on') ||
                                  ...
                                  <ul class="nav nav-pills  mb-3" role="tablist">
                                      ...
                                      @if($company_payment_setting['is_stripe_enabled'] == 'on' && !empty($company_payment_setting['stripe_key']) && !empty($company_payment_setting['stripe_secret']))
                                          <li class="nav-item mb-2">
                                              <a class="btn btn-outline-primary btn-sm"me-1  data-bs-toggle="tab" href="#stripe-payment" role="tab" aria-controls="stripe" aria-selected="true">{{ __('Stripe') }}</a>
                                          </li>
                                      @endif
                                  ...
                                  @endif
                                      ...
                                      @if(isset($company_payment_setting['is_paytr_enabled']) && $company_payment_setting['is_paytr_enabled'] == 'on')
                                          <div class="tab-pane fade" id="paytr-payment" role="tabpanel" aria-labelledby="paytr-payment">
                                              <form role="form" action="{{ route('customer.pay.with.paytr') }}" method="post" class="require-validation" id="paytr-payment-form">
                                                  @csrf
                                                  <input type="hidden" name="invoice_id" value="{{\Illuminate\Support\Facades\Crypt::encrypt($invoice->id)}}">
                                                  <div class="form-group col-md-12">
                                                      <label for="amount">{{ __('Amount') }}</label>
                                                  ...
                                                  <div class="form-group mt-3">
                                                      <button class="btn btn-primary" name="submit"  id="pay_with_paytr" type="submit">{{ __('Make Payment') }}</button>
                                                  </div>
                                              </form>
                                          </div>
                                      @endif
